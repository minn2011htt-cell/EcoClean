...
return H("Các loại vật nuôi & chuồng trại","Chọn vật nuôi để xem chuồng trại, chu kỳ nuôi, bệnh cần lưu ý và hướng xử lý ban đầu.")+
...
C("Bệnh thường gặp & giải pháp theo vật nuôi",`<p class="disease-intro">Các dấu hiệu dưới đây chỉ giúp Sale/CSKH nhận diện tình huống và chuyển đúng người xử lý; không dùng để chẩn đoán qua điện thoại.</p><div class="disease-grid">${(D.diseases[S.animal]||[]).map(x=>`<article class="disease-item"><h4>${x[0]}</h4><p><b>Dấu hiệu cần hỏi:</b> ${x[1]}</p><p><b>Giải pháp ban đầu:</b> ${x[2]}</p><a href="${x[3]}" target="_blank" rel="noopener noreferrer">Nguồn tham khảo thú y ↗</a></article>`).join("")}</div><div class="note"><b>Vai trò của Eco Clean:</b> hỗ trợ quản lý mùi và chất hữu cơ sau khi thu gom phân, vệ sinh và kiểm soát ẩm. Sản phẩm không thay thế cách ly, sát trùng, vaccine, xét nghiệm hoặc điều trị thú y. Khi chết bất thường/chết hàng loạt, ngừng di chuyển đàn và báo thú y địa phương.</div>`)+
